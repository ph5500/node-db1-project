...
router.get("/", (req, res) => {
    db.select("*")
        ...
        .catch(err => {
            console.log(err);
            res.status(500).json({ errorMessage: "Failed to retrieve the list of accounts." });
        });
});
...
router.get("/:id", (req, res) => {
    ...
    db("accounts")
        ...
        .catch(err => {
            console.log(err);
            res.status(500).json({
                errorMessage: "Failed to retreive the account with the specified ID."
            });
        });
});
...
router.post('/', (req, res) => {
    if (!req.body.name) {
        ...
        db("accounts")
            ...
            .catch(err => {
                console.log(err);
                res.status(500).json({ errorMessage: "Failed to add account." });
            });
    }
});
